...
using ValAndInitPos = std::pair<int, int>;
...
int main()
{
    ...
    for (int i = 0; i < vec.size(); ++i)
    {
        ...
        int const val = it->first;
        int newPos = (index + val) % static_cast<int>(vec.size() - 1);
        if (newPos == 0)
        {
            newPos = vec.size() - 1;
        }
    ...
    int const result = vec[(zeroIndex + 1000) % vec.size()].first
