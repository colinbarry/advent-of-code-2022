...
int sumSmallFolderSizes(Folder& folder, int const acc)
{
    return std::accumulate(folder.folders.begin(),
                           folder.folders.end(),
                           folder.size <= 100000 ? acc + folder.size : acc,
                           [](int acc, Folder& each) {
                               return sumSmallFolderSizes(each, acc);
                           });
}
...
int main()
{
    ...
    std::cout << sumSmallFolderSizes(root, 0) << "\n";
}
